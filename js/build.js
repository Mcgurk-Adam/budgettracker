var Database = (function () {
    function Database(onSuccessFunction) {
        ...
        this.dbName = "money";
        this.keyPathString = "transactionId";
        try {
    ...
    Database.updateRow = function (db, tableName, keyIndex, objectToUpdate, successCallback) {
        var idbTransaction = db.transaction([tableName], "readwrite");
        var objectStore = idbTransaction.objectStore(tableName);
        console.log(keyIndex);
        objectStore.openCursor(keyIndex).onsuccess = function (ev) {
            var idbCursor = ev.target.result;
            console.log(ev);
            var oldObject = idbCursor.value;
            if (idbCursor) {
                Object.entries(oldObject).forEach(function (key) {
                    var keyName = key[0];
                    var keyValue = key[1];
                ...
                var idbRequest_1 = idbCursor.update(oldObject);
                idbRequest_1.onsuccess = function (ev) {
                    successCallback(idbRequest_1);
                };
            }
        };
    };
        ...
        Database.fetchAllRowsFromTable(this.db, "transactions", function (idbRequest) {
            ...
            idbRequest.result.forEach(function (transaction) {
                if (currentDate.getFullYear() == transaction.date.getFullYear() && currentDate.getMonth() == transaction.date.getMonth()) {
                    ...
                    logRow_1.querySelector("[data-amount]").setAttribute("data-raw-value", transaction.amount.toString());
                    ...
                    logRow_1.querySelector(".trash").addEventListener("touchstart", function () {
                        var wantToDelete = window.confirm("Are you sure you want to delete this transaction?");
                        if (wantToDelete) {
                            Database.deleteFromDatabase(_this.db, "transactions", transaction.transactionId, function () {
                                _this.calculate();
                                _this.getHistory();
                            });
                        }
                    }, { passive: true });
                    logRow_1.querySelector(".edit").addEventListener("touchstart", function () {
                        Database.fetchRowFromDatabase(_this.db, "transactions", transaction.transactionId, function (idbRequest) {
                            var transactionRow = idbRequest.result;
                            var editModal = document.getElementById("editLogEntryModal");
                        ...
                        }, function () { return console.log("Not able to fetch"); });
                    }, { passive: true });
...
var AppScreen = (function () {
    function AppScreen(pageId, closeClickOnBackground) {
        if (closeClickOnBackground === void 0) { closeClickOnBackground = false; }
...
var modalXButton = document.querySelectorAll("#editLogEntryModal [data-close-modal]");
var editModal = document.getElementById("editLogEntryModal");
var saveEditButton = document.querySelector("#editLogEntryModal [data-save]");
var formElements = editModal.querySelectorAll("form input, form select");
modalXButton.forEach(function (button) {
...
var db = new Database(function (db) {
    ...
    saveEditButton.addEventListener("touchstart", function () {
        var editedObject = {
            amount: 0,
            date: new Date(),
            type: "",
        ...
        Database.updateRow(db, "transactions", document.getElementById("editTransactionId").value, editedObject, function () {
            totals.calculate();
            totals.getHistory();
        });
    }, { passive: true });
});
var reloadButton = document.querySelector("[data-performs-action='reload']");
reloadButton.addEventListener("touchend", reloadApp, false);
function reloadApp() {
    if (window.navigator && navigator.serviceWorker) {
        navigator.serviceWorker.getRegistrations()
            .then(function (registrations) { return registrations.forEach(function (registration) { return registration.unregister(); }); });
    }
